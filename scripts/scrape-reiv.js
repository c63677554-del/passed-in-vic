...
//   node scrape-reiv.js --days=30                     # full run -> ../data.js
//   GEOCODER=mapbox MAPBOX_TOKEN=... node scrape-reiv.js --days=30   (or GEOCODER=google)
// Default geocoder is OSM Nominatim (cached, rate-limited); for CI set a Mapbox/Google key.
...
(async () => {
  ...
  for (const p of out) byKey.set(key(p), p); // fresh run overwrites; keeps un-rescraped past weeks (guides re-attached by enrich-prices)
